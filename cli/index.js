...
function showHelp() {
  ...
  out.line('  project-loop init [--brownfield]    scaffold .loop/ here');
  out.line('');
  out.line(b('Flags'));
...
async function cmdUninstall(flags) {
  ...
  runUninstall(plan);
  out.blank();
  out.dim('  .loop/ directories were left alone. They are project state and part of');
...
function cmdStatus(flags) {
  ...
  const stateFile = path.join(projectRoot, '.loop', 'loop.json');
  if (!fs.existsSync(stateFile)) {
    ...
    try {
      ...
      out.warn('.loop/loop.json exists but could not be parsed: ' + err.message);
    }
  }
